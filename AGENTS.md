...
The long game: become the **trust infrastructure of the informal economy** — a verified-identity + reputation + payments rail that the whole gig economy is built on. See [TRUST_INFRASTRUCTURE_SINGULARITY.md](.Codex/TRUST_INFRASTRUCTURE_SINGULARITY.md).
...
- **`.Codex/sovereigns/`** — 13 domain leaders.
- **`.Codex/geniuses/`** — 65 specialists, grouped by domain.
- **`.Codex/context/`** — durable facts about the market, users, regulations, competitors.
- **`.Codex/rules/`** — hard constraints every agent must respect (security, money, PII, accessibility).
- **`.Codex/memory/`** — decisions and learnings accumulated over the build.
...
- **PII is radioactive.** Aadhaar/VKYC data follows the rules in `.Codex/rules/`. Minimize, encrypt at rest + in transit, never log it, comply with India DPDP Act.
...
Full admin spec: [`.Codex/context/admin_panel.md`](.Codex/context/admin_panel.md).
